			...
			try {
				...
				if (inputStream.available() > 0) {
					...
					if (message instanceof Hello) {
						...
						messageArray[2][1] = new Integer(((Hello) message).getPosx()).toString();
						messageArray[3][0] = "PosY";
						messageArray[3][1] = new Integer(((Hello) message).getPosy()).toString();
						messageArray[4][0] = "Reserved";
						messageArray[4][1] = new Integer(((Hello) message).getReserved()).toString();

						messageHandler.visit((Hello) message, "Tower",
								planeIdString, messageArray);

						if (((Hello) message).getReserved() == 0) {

							message = new Hello(planeID,
									((Hello) message).getPosx(),
									((Hello) message).getPosy(), (byte) 0);
						}
