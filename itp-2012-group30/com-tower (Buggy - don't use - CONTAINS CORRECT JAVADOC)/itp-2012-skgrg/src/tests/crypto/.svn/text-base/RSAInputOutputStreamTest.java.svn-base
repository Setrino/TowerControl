...
/**
 * The Class RSAInputOutputStreamTest.
 */
public class RSAInputOutputStreamTest {

	 /**
 	 * Test encrypting decrypting streams single line.
 	 *
 	...
 	@Test
	    public void testEncryptingDecryptingStreamsSingleLine() throws IOException {
	        for (int i = 4; i < 9; i++) {
	            ...
	            OutputStream out = new RSAOutputStream(pair, dout);
	            ...
	            InputStream in = new RSAInputStream(pair, new DataInputStream(new ByteArrayInputStream(buffer.toByteArray())));
